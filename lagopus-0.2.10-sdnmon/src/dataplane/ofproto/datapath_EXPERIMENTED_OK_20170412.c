...
lagopus_match_and_action(struct lagopus_packet *pkt) {
  ...
  //m_entry not existed in Bloom Filter
  //check monitoringdb: If entry existed, update stats. Else return null.
/*  m_entry = monitoringdb_lookup_m_entry_by_packet(mdb, pkt);
  if(m_entry != NULL){  //If entry existed in monitoringdb, update entry's stats
	  m_result = monitoringdb_m_entry_update_stats(mdb, m_key, pkt);
	  if(m_result != LAGOPUS_RESULT_OK){
		  //printf("Error: monitoring_m_entry_update_stats(...) Failed");
		  goto monitoring_out;
	  }
	  pkt->in_port->bridge->monitoringdb = mdb;
  }
*/
  hash_entry = monitoringdb_lookup_and_update_m_entry(mdb, m_key_str, pkt);
  if(hash_entry != NULL){
  }
  if(hash_entry == NULL){
	  ...
	  if(sampling_decision > 0) { //sample this packet, create a new m_entry and save into monitoringdb
		  ...
		  m_result = monitoringdb_m_entry_add(mdb, pkt, m_key, m_key_str);
		  if(m_result != LAGOPUS_RESULT_OK){
