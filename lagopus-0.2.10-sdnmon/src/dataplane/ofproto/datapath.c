...
lagopus_match_and_action(struct lagopus_packet *pkt) {
   ...
   * @author: Thien X. Phan
   * 2017-02-27
   * Processing for monitoring/anomaly detection
   */
  struct monitoringdb *mdb;
  struct bloom_filter *bloom_filter;
  struct Hash_Entry *hash_entry;
  lagopus_result_t m_result;
  int sampling_decision;
  char *m_key_str=NULL;
  //struct m_key *m_key;
  uint32_t srcIp;
  uint16_t srcPort;
  ...
  unsigned int entry_hash_value;
  uint64_t byte_count_of_arriving_packet;

  mdb = pkt->in_port->bridge->monitoringdb;
  bloom_filter = pkt->in_port->bridge->bloom_filter;
/*m_key = extract_m_key_from_packet(pkt);
  if(m_key == NULL)
	  goto monitoring_out;
*/
  rv = extract_5tuple_from_packet(pkt, &srcIp, &srcPort, &dstIp, &dstPort, &proto);

  //CHECK Bloom_filter
  //Use hash funtion to create (unsigned int) hash_value of m-entry

  //entry_hash_value = hash_m_key(m_key);
  entry_hash_value = hash_5tuple(srcIp, srcPort, dstIp, dstPort, proto);

  //int bloom_check = bloom_filter_check(bloom_filter, m_key_str);
  int bloom_check = bloom_filter_check(bloom_filter, entry_hash_value);
  if(bloom_check != 0){  //m_entry existed in Bloom Filter, not monitor this entry
	  goto monitoring_out;
  }

  byte_count_of_arriving_packet = OS_M_PKTLEN(PKT2MBUF(pkt));
  //printf("1\n");
  //hash_entry = monitoringdb_lookup_and_update_m_entry(mdb, entry_hash_value, byte_count_of_arriving_packet);
  hash_entry = HashTable_Find_And_Update_Entry(mdb->hash_table, byte_count_of_arriving_packet, entry_hash_value);
  //printf("2\n");
  if(hash_entry == NULL){
	  if(mdb->sampling_ratio == 1)
		  sampling_decision = 1;
	  else
		  sampling_decision = sampling_check(mdb->sampling_ratio);
	  if(sampling_decision > 0) { //sample this packet, create a new m_entry and save into monitoringdb
		  mdb->n_flows++;  //for debugging
		  //printf("3\n");
		  //m_result = monitoringdb_m_entry_add(mdb, byte_count_of_arriving_packet, m_key, entry_hash_value);
		  //m_result = monitoringdb_m_entry_add(mdb, byte_count_of_arriving_packet, srcIp, srcPort, dstIp, dstPort, proto, entry_hash_value);
		  hash_entry = HashTable_Add_Entry(mdb->hash_table, byte_count_of_arriving_packet, srcIp, srcPort, dstIp, dstPort, proto, entry_hash_value);
		  //printf("4\n");
		  if(hash_entry == NULL){ //if(m_result != LAGOPUS_RESULT_OK){
			  printf("Error: monitoringdb_m_entry_add(...) Failed\n");
			  goto monitoring_out;
		  }
		  pkt->in_port->bridge->monitoringdb = mdb;
	  }
	  else{ //Ignore this packet/flow, mark it into Bloom filter as non-monitoring flow
		  //bloom_filter_insert(bloom_filter, m_key_str);
		  bloom_filter_insert(bloom_filter, entry_hash_value);
		  mdb->n_bloom_members++; //for debugging
	  }
  }

/*    printf("\nMonitoring database: \n");
  printf("{srcIP, dstIP, proto, packet count, byte count}\n");
  for (int i = 0; i < mdb->m_table->nm_entry; i++) {
  	  	  	  	  printf(" m_entry[%d]: ", i);
      				printf("{ %s, ", inet_ntoa(mdb->m_table->m_entries[i]->m_key->srcIp));
      				printf(" dstIp: %s, ", inet_ntoa(mdb->m_table->m_entries[i]->m_key->dstIp));
      				printf(" proto: %d, ", mdb->m_table->m_entries[i]->m_key->proto);
      for(int j=0; j < mdb->m_table->m_entries[i]->n_stats; j++){
      				printf(" (%" PRIu64 ", ", mdb->m_table->m_entries[i]->stats_list[j]->packet_count);
      			  printf(" %" PRIu64 ")", mdb->m_table->m_entries[i]->stats_list[j]->byte_count);
      }
      printf(" }\n");
  }
