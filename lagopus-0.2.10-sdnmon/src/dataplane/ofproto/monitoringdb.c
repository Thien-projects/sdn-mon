...
struct monitoringdb *monitoringdb_alloc(){
	  struct monitoringdb *monitoringdb;
	  int numBuckets = 1024; //65536; //32768; //16384; //8192; //4096;
	  int outerNumBuckets = 1024;
	  //uint8_t i;
...
*/
	  monitoringdb->outer_hash_table = allocHashTable(outerNumBuckets, numBuckets); //table size = 2 luy thua 12 = 4096 //allocHashTable(MONITORINGDB_TABLE_SIZE_MAX);
	  	  if (monitoringdb->outer_hash_table == NULL) {
	  		  monitoringdb_free(monitoringdb);
	  		  return NULL;
	  	  }
	 ...
	 if(monitoringdb != NULL) {
		printf("SDN-Mon initialized successfully...\n");
	 ...
	 printf("Sampling ratio: %d\n", monitoringdb->sampling_ratio);
...
monitoringdb_free(struct monitoringdb *monitoringdb){
      if (monitoringdb->outer_hash_table != NULL) {
        Hash_DeleteTable(monitoringdb->outer_hash_table);
        monitoringdb->outer_hash_table = NULL;
      }
	  // Free monitoringdb.
	  free(monitoringdb);
}
...
*/
	...
	*srcIp=pkt->ipv4->ip_src.s_addr;
	*dstIp=pkt->ipv4->ip_dst.s_addr;
	*proto=IPV4_PROTO(pkt->ipv4);
	*srcPort = 65535; //65535 means srcPort, dstPort has NULL value
	*dstPort = 65535;
	...
	if(*proto == 6){
		//printf("TCP packet\n");
		*srcPort=TCP_SPORT(pkt->tcp);
		*dstPort=TCP_DPORT(pkt->tcp);
		return ret;
	}
	if(*proto == 17){
		//printf("UDP packet\n");
		*srcPort=UDP_SPORT(pkt->udp);
		*dstPort=UDP_DPORT(pkt->udp);
		return ret;
	}
...
monitoringdb_m_entry_add(struct monitoringdb *mdb, uint64_t byte_count_of_arriving_entry, struct m_key *key, unsigned int entry_hash_value){
	...
	newPtr = false;
	Hash_Entry *e;
	e = HashTable_Add_Entry(mdb->hash_table, byte_count_of_arriving_entry, key, entry_hash_value, newPtr);
	if(e == NULL) //(newPtr == false)
		ret = LAGOPUS_RESULT_OFP_ERROR;

	mdb->n_m_entries = mdb->hash_table->numEntries;
	//if(mdb->n_m_entries % 20 == 0)
	//	printf("monitoringdb.c-monitoringdb_m_entry_add() - mdb->n_m_entries= %d\n", mdb->n_m_entries);
	//printf(" %s\n",m_key_str);
...
monitoringdb_m_entry_add(struct monitoringdb *mdb, uint64_t byte_count_of_arriving_entry, uint32_t srcIp, uint16_t srcPort, uint32_t dstIp, uint16_t dstPort, uint8_t proto, unsigned int entry_hash_value){
	...
	//newPtr = false;
	Hash_Entry *e;
	e = HashTable_Add_Entry(mdb->hash_table, byte_count_of_arriving_entry, srcIp, srcPort, dstIp, dstPort, proto, entry_hash_value); //, newPtr);
	if(e == NULL) //(newPtr == false)
		ret = LAGOPUS_RESULT_OFP_ERROR;
...
insert_m_entry_from_controller(struct monitoringdb *mdb, uint32_t src_ip, uint16_t src_port, uint32_t dst_ip, uint16_t dst_port, uint8_t proto){
	...
	//m_key_str = m_key_to_string(key);
	entry_hash_value = hash_5tuple(src_ip, src_port, dst_ip, dst_port, proto);
	e = HashTable_Add_Entry_From_Controller(mdb->hash_table, src_ip, src_port, dst_ip, dst_port, proto, entry_hash_value, newPtr);
	if(e == NULL) //(newPtr == false)
		ret = LAGOPUS_RESULT_OFP_ERROR;
...
struct Hash_Entry *monitoringdb_lookup_and_update_m_entry(struct monitoringdb *mdb, unsigned int entry_hash_value, uint64_t byte_count_of_arriving_packet){
	return HashTable_Find_And_Update_Entry(mdb->hash_table, byte_count_of_arriving_packet, entry_hash_value);
}
...
#define COPY_M_KEY(member) m_entry_stats->m_key.member = e->member;
	COPY_M_KEY(srcIp);
	COPY_M_KEY(dstIp);
	COPY_M_KEY(proto);
	if(e->srcPort != NULL){
...
hashTable_Get_Entry_Stats(struct Hash_Table *t, struct m_entry_stats_list *m_entry_stats_list){
	...
	if(e != NULL){
		//ONLY pick up updated entry or new entry (check_updated_entry == 0 or 1) to the list.
		if(e->check_updated_entry != 2){
			...
			//Then mark it as an non-updated entry (set check_updated_entry = 2)
			e->check_updated_entry = 2;
		}
	}
	while(e != NULL){
		...
		if(e != NULL){
			...
			if(e->check_updated_entry != 2){
				...
				//Then mark the entry as an non-updated entry (set check_updated_entry = 2)
				e->check_updated_entry = 2;
			}
		}
	}
...
unsigned int hash_m_key(struct m_key *m_key){
	unsigned int hash = 0;
	hash += (m_key->srcIp >> 16) | (m_key->srcIp << 16); //rotate srcIp by 16 bits
	hash += (m_key->dstIp >> 8) | (m_key->dstIp << 24);
	hash += (m_key->proto >>4) | (m_key->proto << 4);
	if(m_key->srcPort != NULL)
		hash += (m_key->srcPort >> 8) | (m_key->srcPort << 8);
	if(m_key->dstPort != NULL)
		hash += (m_key->dstPort >> 4) | (m_key->dstPort << 12);
...
{
	unsigned int hash = 0;
	hash += (srcIp >> 16) | (srcIp << 16); //rotate srcIp by 16 bits
	hash += (dstIp >> 8) | (dstIp << 24);
	hash += (proto >>4) | (proto << 4);
	if(srcPort != NULL)
		hash += (srcPort >> 8) | (srcPort << 8);
	if(dstPort != NULL)
		hash += (dstPort >> 4) | (dstPort << 12);
	hash = hash & 0xFFFFFFFF;
	return hash;
}
