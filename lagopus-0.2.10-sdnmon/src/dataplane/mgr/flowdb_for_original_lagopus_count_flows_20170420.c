...
struct flowdb {
   ...
   * 2017-04-20
   */
  uint32_t flow_count;  //for counting number of flows for experiments
  //
};
...
flowdb_flow_add(struct bridge *bridge,
   ...
   */
  if(ret == LAGOPUS_RESULT_OK){
	  bridge->flowdb->flow_count++;
	  if(bridge->flowdb->flow_count % 50 == 0)
	  	  printf("#flow entries (bridge->flowdb->flow_count): %d\n", bridge->flowdb->flow_count);
  }
  //
...
flowdb_flow_delete(struct bridge *bridge,
  ...
  if(result == LAGOPUS_RESULT_OK){
	  bridge->flowdb->flow_count--;
	  //if(bridge->flowdb->flow_count % 10 == 0)
	  	  printf("[Flow entry REMOVED] #flow entries (bridge->flowdb->flow_count): %d\n", bridge->flowdb->flow_count);
  }
  //
