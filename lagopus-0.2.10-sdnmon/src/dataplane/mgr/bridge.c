...
bridge_alloc(const char *name) {
   ...
   * 2017-02-27
   */
  bridge->monitoringdb = monitoringdb_alloc();
  if (bridge->monitoringdb == NULL) {
  	goto out;
  }
  bridge->bloom_filter = bloom_filter_alloc();
  if (bridge->bloom_filter == NULL) {
  	goto out;
  }
  //
...
bridge_free(struct bridge *bridge) {
  ...
  */
  if (bridge->monitoringdb != NULL) {
	  monitoringdb_free(bridge->monitoringdb);
  }
  if (bridge->bloom_filter != NULL){
	  bloom_filter_free(bridge->bloom_filter);
  }
  //
