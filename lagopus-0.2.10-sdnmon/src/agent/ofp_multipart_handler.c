...
ofp_multipart_request_handle(struct channel *channel, struct pbuf *pbuf,
  ...
  switch (multipart.type) {
    ...
    case OFPMP_EXPERIMENTER:
      ...
      //res = ofp_bad_experimenter_handle(error);  //Commented from original code
    	 ...
    	 */
    	printf("\n#--------------------------------------------------------#\n");
    	printf("\nReceived SDNMON_MULTIPART_REQUEST from controller.\n");
    	res = ofp_experimenter_mp_request_handle(channel, all, xid_header, error);
    	//
