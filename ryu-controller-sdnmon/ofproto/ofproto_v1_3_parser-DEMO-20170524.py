...
# from ryu.app import sdnmon_stats_monitor
# from sdnmon_stats_monitor import SDNMONMonitor
...
class OFPMultipartReply(MsgBase):
    ...
    #list of transaction_id of replies that has >1 messages in its multipart-messages sequence
    global multipart_xid_list 
    multipart_xid_list = []
    ...
    def __init__(self, datapath, body=None, flags=None):
        ...
        #testing: list of transaction_id of replies that has >1 messages in its multipart-messages sequence
        #self.multipart_xid_list = []

    @classmethod
    def parser_stats_body(cls, buf, msg_len, offset):
        
        #print('OFPMultipartReply.parser_stats_body(...) called')
    ...
    def parser_stats(cls, datapath, version, msg_type, msg_len, xid, buf):
        ...
        #print('OFPMultipartReply.parser_stats(...) called')
        ...
        msg.body = msg.parser_stats_body(msg.buf, msg.msg_len,
                                        ofproto.OFP_MULTIPART_REPLY_SIZE) #ofproto.SDNMON_MULTIPART_REPLY_SIZE)
        return msg

    @classmethod
    def parser(cls, datapath, version, msg_type, msg_len, xid, buf):
        
        #print('OFPMultipartReply.parser(...) called')
        
        type_, flags = struct.unpack_from(
            ofproto.OFP_MULTIPART_REPLY_PACK_STR, six.binary_type(buf), #ofproto.SDNMON_MULTIPART_REPLY_PACK_STR, six.binary_type(buf),
            ofproto.OFP_HEADER_SIZE)
        ...
        #offset = ofproto.SDNMON_MULTIPART_REPLY_SIZE

        # Doan code tren parse phan header cua ofp_multipart_reply:
        # { ofp_header (8bytes), types (2bytes), flags (2bytes), 4 byte dem (pad)
        
        #test
        #print('msg.flags = %d', msg.flags)
        #print('xid = %d', xid)
        #test: parsing the fields: experimenter_type (4 bytes), experimenter_id (4 bytes)
        if stats_type_cls.cls_stats_body_cls == OFPSDNMonStats: #OFPExperimenterMultipart: #ofproto.OFPMP_EXPERIMENTER:
            if not multipart_xid_list.__contains__(xid):
                if msg.flags == 1: #more follow-up multipart-messages of this reply (xid) will come consequently 
                    multipart_xid_list.append(xid)
                #print('stats_type_cls == ofproto.OFPMP_EXPERIMENTER (parsing experimenter_type, experimenter_id here!)')
                    #parsing 2 bien: {experimenter (4bytes), exp_type (4bytes)} => args
                ...
                #print('experimenter: %d', args[0])
                #print('exp_type: %d', args[1])
                ...
                numbers_of_entries = struct.unpack_from(
                    NUMBERS_OF_ENTRIES_BUCKET_COUNTER_PACK_STR, buf, offset)
                offset += NUMBERS_OF_ENTRIES_BUCKET_COUNTER_SIZE
                number_of_flows = numbers_of_entries[0]
                number_of_m_entries = numbers_of_entries[1]
                #print('Switch: ', msg.datapath.id,' #flows = ', number_of_flows, '#m_entries: ', number_of_m_entries)
               
            #End of parsing experimenter, exp_type, number_of_flows, number_of_m_entries    
            ########################
            else:
                if msg.flags == 0: #last message of the sequence of that reply (xid)
                    multipart_xid_list.remove(xid)
                    
        body = [] #Day chinh la phan experimenter_multipart body gui tu switch! {experimenter, exp_type, list cac m_entry_stats} 
        while offset < msg_len:
            #print('offset < msg_len')
            b = stats_type_cls.cls_stats_body_cls.parser(msg.buf, offset)  #parse moi experimenter_multipart body gom {list cac m_entry_stat, experimenter (4bytes), exp_type (4bytes)} nhu the nao???
            body.append(b)
        ...
        if stats_type_cls.cls_body_single_struct:
            ...
            msg.body = body
        return msg
    #sau khi parse o ham nay, msg.body se la experimenter_multipart body {experimenter, exp_type, list cac m_entry_stats}
    #phan body nay se duoc parsed tiep o ham parse cua class ExperimenterMultipartReply! 
...
#@THIEN: 2016-11-22
class OFPSDNMonStats(StringifyMixin):
    def __init__(self, packet_count=None, byte_count=None, proto=None, 
                 src_port=None, dst_port=None, 
                 src_ip=None, dst_ip=None, #,
                 length=None):
        super(OFPSDNMonStats, self).__init__()
        self.length = 32 #0 #8
        self.packet_count = packet_count
        self.byte_count = byte_count
        self.proto = proto
    ...
    def parser(cls, buf, offset):
        ...
        m_entry_stats = cls()
        #print("OFPSDNMonStats.parser() called")
        #OFP_M_ENTRY_STATS_0_PACK_STR = '!Q'
        #OFP_M_ENTRY_STATS_0_SIZE = 8
        #assert calcsize(OFP_M_ENTRY_STATS_0_PACK_STR) == OFP_M_ENTRY_STATS_0_SIZE
        M_ENTRY_BUCKET_COUNTER_PACK_STR = '!QQ3xBHHII' 
        M_ENTRY_BUCKET_COUNTER_SIZE = 32 # 16 bytes: packet_count+byte_count, 4 bytes: srcPort+dstPort, 1 byte: proto, 8bytes: srcIp+dstIp (uint32_t), 
...
#         (m_entry_stats.packet_count
#         #m_entry_stats.byte_count
#          ) = struct.unpack_from(
#             OFP_M_ENTRY_STATS_0_PACK_STR, buf, offset)
#         offset += OFP_M_ENTRY_STATS_0_SIZE
        m_entry_stats.length = 32
        (m_entry_stats.packet_count, m_entry_stats.byte_count, m_entry_stats.proto, 
         m_entry_stats.src_port, m_entry_stats.dst_port, 
         m_entry_stats.src_ip, m_entry_stats.dst_ip) = struct.unpack_from(
                M_ENTRY_BUCKET_COUNTER_PACK_STR, buf, offset)
        offset += M_ENTRY_BUCKET_COUNTER_SIZE
        
        return m_entry_stats
#@THIEN: 2016-11-22
...
class OFPExperimenterMultipart(ofproto_parser.namedtuple(
    ...
    def parser(cls, buf, offset):
        ...
        #print('OFPExperimenterMultipart.parser(...) called')
        
        #parsing 2 bien: {experimenter (4bytes), exp_type (4bytes)} => args
        args = struct.unpack_from(
            ofproto.OFP_EXPERIMENTER_MULTIPART_HEADER_PACK_STR, buf,
            offset)
        args = list(args)   #args=[experimenter (4bytes), exp_type (4bytes)]
        
        #testing parse number_of_flow_entry (4 bytes), number_of_m_entry (4 bytes)
#         offset = offset + ofproto.OFP_EXPERIMENTER_MULTIPART_HEADER_SIZE
#         numbers_of_entries = struct.unpack_from(
#             ofproto.OFP_EXPERIMENTER_MULTIPART_HEADER_PACK_STR, buf,
#             offset)
#         numbers_of_entries = list(numbers_of_entries)
#         args.append(numbers_of_entries)
        ###
        
        args.append(buf[offset +
                         ofproto.OFP_EXPERIMENTER_MULTIPART_HEADER_SIZE:]) #data: list cac m_entry_stats la day!
        
        #args=[experimenter (4bytes), exp_type (4bytes), buf (phan DATA)] !!! Data nay = m_entry_stats (chi 1, ko phai mot list)
        
        #Thu in ra dang hex cua data?
        #Thu parse m_entry_stat o day??!!!
        
        
        #end parsing m_entry_stats
        
        stats = cls(*args)
        stats.length = ofproto.OFP_METER_FEATURES_SIZE #???
        return stats  #stats == list of m_entry_stat
...
#Thien - 2016-06-16 => 2016-11-22
#Modified class: OFPExperimenterMultipart to parse monitoring statistics and m_key 
#in OFPExperimenterReply received from switch 
class OFPSDNMonMultipart(ofproto_parser.namedtuple(
                               'OFPSDNMonMultipart',
                               ('experimenter', 'exp_type', 'data'))):
    """
    The body of OFPExperimenterStatsReply multipart messages.
    ...
    def parser(cls, buf, offset):
        ...
        print('OFPSDNMonMultipart.parser() called')
        args = struct.unpack_from(
            ofproto.OFP_EXPERIMENTER_MULTIPART_HEADER_PACK_STR, buf,
            offset)
        args = list(args)
        ...
        #m_entry_stats = OFPSDNMonStats.parser(buf, offset)
        OFP_M_ENTRY_STATS_0_PACK_STR = '!Q'
        OFP_M_ENTRY_STATS_0_SIZE = 8
        assert calcsize(OFP_M_ENTRY_STATS_0_PACK_STR) == OFP_M_ENTRY_STATS_0_SIZE
        #m_entry_stats = cls()
        ...
        args.append(m_entry_stats)
        stats = cls(*args) #???
        stats.length = 24 #ofproto.OFP_METER_FEATURES_SIZE
        return stats #m_entry_stats #
################
...
#Thien - 2016-06-22
# class OFPExperimenterMultipart(StringifyMixin):
#     """
#     The body of OFPExperimenterStatsReply multipart messages.
#   
#     ================ ======================================================
...
#     """
#      
#     _SDNMON_ENTRY_STATS_SIZE = 16 # 8 bytes of packet_count (uint64_t), 8 bytes of byte_count
#     _SDNMON_DATA_PACK_STR = '!I'
#      
#     def __init__(self, #experimenter=None, exp_type=None, data=None, 
#                     packet_count=None, byte_count=None): # match=None, instructions=None):
#         super(OFPExperimenterMultipart, self).__init__()
# #         self.experimenter=experimenter
# #         self.exp_type=exp_type
# #         self.data=data
#         self.packet_count = packet_count
...
#         stats = cls()
#  
#         (stats.packet_count, stats.byte_count) = struct.unpack_from(
#             cls._SDNMON_DATA_PACK_STR, buf, offset)
#         offset += cls._SDNMON_ENTRY_STATS_SIZE #ofproto.OFP_FLOW_STATS_0_SIZE
#  
# #         flow_stats.match = OFPMatch.parser(buf, offset)
# #         match_length = utils.round_up(flow_stats.match.length, 8)
# #         inst_length = (flow_stats.length - (ofproto.OFP_FLOW_STATS_SIZE -
...
@_set_stats_type(ofproto.OFPMP_EXPERIMENTER, OFPExperimenterMultipart)
@_set_msg_type(ofproto.OFPT_MULTIPART_REQUEST)
class OFPSDNMonStatsRequest(OFPExperimenterStatsRequestBase):
    """
    Experimenter multipart request message
    ...
    def __init__(self, datapath, flags,
                 experimenter, exp_type, data,
                 type_=None):
        super(OFPSDNMonStatsRequest, self).__init__(datapath, flags,
                                                          experimenter,
                                                          exp_type, type_)
        self.data = data
...
#@OFPMultipartReply.register_stats_type()  #(body_single_struct=False) #True)

@OFPMultipartReply.register_stats_type() #@OFPMultipartReply.register_stats_type(body_single_struct=True)
@_set_stats_type(ofproto.OFPMP_EXPERIMENTER, OFPSDNMonStats) #OFPExperimenterMultipart) #OFPSDNMonMultipart)  #OFPSDNMonStats)
@_set_msg_type(ofproto.OFPT_MULTIPART_REPLY)
...
@_set_stats_type(ofproto.OFPMP_EXPERIMENTER, OFPExperimenterMultipart)
@_set_msg_type(ofproto.OFPT_MULTIPART_REPLY)
class OFPSDNMonStatsReply(OFPMultipartReply):
    """
    SDN-Mon (Experimenter) multipart reply message
    ...
    """
    def __init__(self, datapath, type_=None, **kwargs):
        super(OFPSDNMonStatsReply, self).__init__(datapath, **kwargs)
