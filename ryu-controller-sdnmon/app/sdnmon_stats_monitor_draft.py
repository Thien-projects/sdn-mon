...
#THIEN: 2016-05-04
    ...
    def _experimenter_mp_handler(self, ev):
        ...
        for stat in sorted(body):
            ...
            offset = 0
            while m_entry_stats_length > 0:
                #unpack a m_entry_stats from buf
                m_entry_stats = struct.unpack_from(
                    ofproto.OFP_BUCKET_COUNTER_PACK_STR, data, offset)
                ...
                offset += 16 #ofproto.OFP_FLOW_STATS_0_SIZE
                
                m_entry_stats_list.append(m_entry_stats)
                m_entry_stats_length -= 16;
                
            
        self.logger.info('number of entry in m_entry_stats_list: %8d', len(m_entry_stats_list))
        for m_entry_stats in m_entry_stats_list:
            self.logger.info('-Packet_count: %i  -Byte_count: %i', m_entry_stats[1], m_entry_stats[0])
