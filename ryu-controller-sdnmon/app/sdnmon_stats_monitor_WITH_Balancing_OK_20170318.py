...
#                 self.logger.info('#################################')
                self.logger.info('Global Monitoring Tables: (#total_m_entries: %d)', len(self.buffering_table))
                for t in self.global_m_tables.keys():
                    switch_table = self.global_m_tables[t]
                    print('GMT-Switch: ', t, '  #entry: ', len(switch_table))
...
#                 with open('/home/thien/ryu-controller-experiment-data/test3.csv', 'wb') as csv_file:
...
#                     for t in self.global_m_tables.keys():
#                         number_entries = len(self.global_m_tables[t])
#                         data_row.append(number_entries)
#                         total_entries += number_entries
#                         f.write(' %d', number_entries)
#                         #writer.writerow([key, value])
#                     data_row.append(total_entries)
# 
#                     writer.writerow(data_row)
#                     f.write(' %d', total_entries)
                    #csv_file.close()
                    
                    #for t in self.global_m_tables.keys():
                    #    number_entries = len(self.global_m_tables[t])
                    #    writer.writerow([(number_entries * 100)/total_entries])
                #self.logger.info('EXPORTED monitoring database to CSV file')
            #update Load Status Table (Switch Memory Usage Table) 
