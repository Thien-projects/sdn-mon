            ...
            if ip is not None:
                ...
                if t is not None:
                    ...
                    match = parser.OFPMatch(ipv4_src=nw_src, ipv4_dst=nw_dst, ip_proto=nw_proto, 
                                            tcp_src=tp_src, tcp_dst=tp_dst, eth_type=eth.ethertype) #ether_types.ETH_TYPE_IP) #
                    self.logger.info("IPv4 packet - TCP: %s %s %s %s %s", nw_src, nw_dst, nw_proto, tp_src, tp_dst)
                ...
                if u is not None:         
                    ...
                    tp_dst = u.dst_port
                    match = parser.OFPMatch(ipv4_src=nw_src, ipv4_dst=nw_dst, ip_proto=nw_proto, 
                                            udp_src=tp_src, udp_dst=tp_dst, eth_type=eth.ethertype)
                    self.logger.info("IPv4 packet - UDP: %s %s %s %s %s", nw_src, nw_dst, nw_proto, tp_src, tp_dst)
                ...
                if ic is not None:
                    ...
                    tp_dst = ic.code
                    match = parser.OFPMatch(ipv4_src=nw_src, ipv4_dst=nw_dst, ip_proto=nw_proto, 
                                            icmpv4_type=tp_src, icmpv4_code=tp_dst, eth_type=eth.ethertype)
                    self.logger.info("IPv4 packet - ICMP: %s %s %s %s %s", nw_src, nw_dst, nw_proto, tp_src, tp_dst)
#-------            
                ...
                if match is not None:                   
                    self.logger.info("match is not None (IPv4 packet)")
