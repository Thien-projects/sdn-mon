...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    n_flows = 0 #for counting number of 5-tuple flows installed to switch
    ipv4_packet_count = 0
    ipv6_packet_count = 0
    non_ipv4_ipv6_packet_count = 0
...
#@Thien: 2016-05-22
            ...
            ip = pkt.get_protocol(ipv4.ipv4)
            if ip is not None:
                self.ipv4_packet_count += 1
                if (self.ipv4_packet_count % 100 == 0):
                    self.logger.info("#Ipv4-packets: %d", self.ipv4_packet_count)
                nw_src = ip.src
                nw_dst = ip.dst
                nw_proto = ip.proto
                ...
                if t is not None:
                    tp_src = t.src_port
                    tp_dst = t.dst_port
                    match = parser.OFPMatch(ipv4_src=nw_src, ipv4_dst=nw_dst, ip_proto=nw_proto, 
                                            tcp_src=tp_src, tcp_dst=tp_dst, eth_type=ether_types.ETH_TYPE_IP) #eth.ethertype)
                ...
                if u is not None:         
                    ...
                    tp_dst = u.dst_port
                    match = parser.OFPMatch(ipv4_src=nw_src, ipv4_dst=nw_dst, ip_proto=nw_proto, 
                                            udp_src=tp_src, udp_dst=tp_dst, eth_type=ether_types.ETH_TYPE_IP) #eth.ethertype)
...
#                                             udp_src=tp_src, udp_dst=tp_dst, eth_type=ether_types.ETH_TYPE_IP) #eth.ethertype)
                ...
                if ic is not None:
                    ...
                    tp_dst = ic.code
                    match = parser.OFPMatch(ipv4_src=nw_src, ipv4_dst=nw_dst, ip_proto=nw_proto, 
                                            icmpv4_type=tp_src, icmpv4_code=tp_dst, eth_type=ether_types.ETH_TYPE_IP) #eth.ethertype)
...
#-------            
                ...
                if match is not None:                   
                    ...
                    if(self.n_flows % 100 == 0):
                        self.logger.info("#flow-entries installed: %d", self.n_flows)
                    ###
                ...
                if ip6 is not None:
                    self.ipv6_packet_count += 1
                    if (self.ipv6_packet_count % 100 == 0):
                        self.logger.info("    #IPV6-packets: %d", self.ipv6_packet_count)
                else: 
                    self.non_ipv4_ipv6_packet_count += 1
                    if (self.non_ipv4_ipv6_packet_count % 100 == 0):
                        self.logger.info("NON_Ipv4_Ipv6_packets: %d", self.non_ipv4_ipv6_packet_count)
                #self.non_ipv4_packet_count += 1
                #if(self.non_ipv4_packet_count % 100 == 0):
                #    self.logger.info("Ignored (Not IPv4 tcp/udp/icmp) packets : %d", self.non_ipv4_packet_count)
