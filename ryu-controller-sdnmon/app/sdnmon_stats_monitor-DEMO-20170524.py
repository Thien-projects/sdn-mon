        ...
        while True: 
            ...
            if((current_time - self.query_time) == (self.query_time_interval - 2)):
                ...
                for entry_hash in self.buffering_table.keys():
                    switch_id = self.buffering_table[entry_hash]["sw_id"]
                    self.global_m_tables[switch_id][entry_hash] = self.buffering_table[entry_hash]
#                     entry_buf_table = self.buffering_table[entry_hash]
#                     switch_id = self.buffering_table[entry_hash]["sw_id"]
#                     entry_gmt = self.global_m_tables[switch_id].get(entry_hash)
#                     if entry_gmt == None:
#                         self.global_m_tables[switch_id][entry_hash] = entry_buf_table #self.buffering_table.get(entry_hash)
#                     else: 
...
#                 self.logger.info('#################################')
                ...
                for t in self.global_m_tables.keys():
                    ...
                    self.logger.info('GMT-Switch: %d  #Monitoring-entries: %d', t, len(switch_table))
#                     for k in switch_table.keys():
#                         print('Entry_hash:', k, ' Entry:', switch_table[k])
                    self._export_monitoring_data_to_csv('/home/thien/ryu-controller/monitoring_data/monitoring_data.csv')
                self.logger.info('Exported monitoring data update to CSV file:')
                self.logger.info('/home/thien/ryu-controller/monitoring_data/monitoring_data.csv')
                self.logger.info('#----------------------------------------------#')
                self.logger.info('')
...
#         self.logger.info('                          Sent EXPERIMENTER Request (setSamplingRatio) to Switch.')
    ...
    def _export_monitoring_data_to_csv(self, path_to_csv_file):
        ...
        with open(path_to_csv_file, 'wb') as csv_file:
            ...
            for t in self.global_m_tables.keys():
                m_table = self.global_m_tables[t]
                for key, value in m_table.items():
                    writer.writerow([key, value])
                    
        #self.logger.info('EXPORTED monitoring data to CSV file')
...
#         self.logger.info('#Entry received from Switch %d: %u', ev.msg.datapath.id, m_entries_count)
        ...
        for m_entry in sorted(body):
            ...
            #Check and process for updated entries here!
