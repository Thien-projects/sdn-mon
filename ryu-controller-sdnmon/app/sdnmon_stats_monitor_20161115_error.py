...
#THIEN: 2016-05-04
    ...
    def _experimenter_mp_handler(self, ev):
        ...
        #test printing out m_entry_stats_list received in Experimenter Reply from switch
        body = ev.msg.body
        #self.logger.info('datapath         ' 'experimenter_id  experimenter_type   packets   bytes')
        self.logger.info('datapath_id        ' 'src_port        dst_port     packets       bytes')
      
        M_ENTRY_BUCKET_COUNTER_PACK_STR = '!QQ' #'!HHQQ'  # '!HHBQQ' #'!QQ'
        M_ENTRY_BUCKET_COUNTER_SIZE = 16 #32 # 16 bytes: packet_count+byte_count, 4 bytes: srcPort+dstPort, 1 byte: proto
        ...
        for stat in sorted(body):
            ...
            #append 5-tuples to m_entry here!
            
            data=stat.data
            #self.logger.info('Stat data: %s', data)
            
            m_entry_stats_length = data.__len__()
            #self.logger.info('Stat data length: %d', m_entry_stats_length)
            
            while (m_entry_stats_length - offset > 0):
                ...
                m_entry_stats = struct.unpack_from(
                    M_ENTRY_BUCKET_COUNTER_PACK_STR, data, 0)
                    
                m_entry_stats = list(m_entry_stats)
                #m_entry_stats = [x.rstrip(b'\0') for x in m_entry_stats]
                #stats = cls(*m_entry_stats)
                #self.logger.info('number of items in m_entry_stats: %8d', len(m_entry_stats))
                ...
                offset += 16 #32
                m_entry_stats_length -= 16 #32;
                
            
        for m_entry in m_entry_list:
            self.logger.info('%016x    %i    %i    ', m_entry[0],  
                             m_entry[1][1], m_entry[1][0]) #m_entry[1][2], m_entry[1][1], m_entry[1][3], m_entry[1][0]) 
        self.logger.info('number of entry in m_entry_list: %8d', len(m_entry_list))
